...
const createPatient=async(req,res)=>{
    try{
       ...
       if(!isValid(phone)) return res.status(400).send({message: 'phone no is required' });
        ...
        if (!phone.match(phoneRex)) return res.status(400).send({ message: `Please fill Indian phone number` })
       ...
       if (!isValid(email)) return res.status(400).send({message: `Email is required` })
        ...
        // images 
        let patientImage={data:fs.readFileSync('uploads/'+req.file.filename),contentType:"image/png"}

        console.log(patientData)
        patientData.patientImage=patientImage


        const patientCreated=await patientModel.create(patientData)
